...
func get_valid_rand_coords():
	var rand_x = randi_range(0, grid_size.x)
	var rand_y = randi_range(0, grid_size.y)
	...
	while not is_free_cell(coords, list_pos_occupied):
		rand_x = randi_range(0, grid_size.x)
		rand_y = randi_range(0, grid_size.y)
